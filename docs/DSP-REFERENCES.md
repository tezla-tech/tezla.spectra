...
| N. H. Fletcher & T. D. Rossing, *The Physics of Musical Instruments* (Springer) | book; **READ first-hand since 2026-09-05** -- the user supplied the complete text to `technical references/drumsynth/new/`; the bell chapter (21) has not yet been read against this row, see the Ictus section's chapter map | The church bell's minor-third partial series (hum 1/2, prime 1, tierce 1.2, quint 1.5, nominal 2, ...) -- an empirical founders' profile with no closed form, trusted through the standard organology literature and marked as such at the point of use. Everything else those chapters cover is derived instead |
...
## Drum synthesis — Ictus

Ictus synthesises drums from mechanism — the published circuit analyses of two
classic machines, membrane physics, and the classic subtractive recipes — and
never from any product's binary, samples or presets. Every source below was
**blocked at the network layer** on the first attempt (qub.ac.uk, dafx.de,
...
record -- and it happened again on 2026-09-05 for the hats' plate: the six
cymbal-physics sources below (J-STAGE, HAL, ScienceDirect, AIP, ResearchGate,
Sound On Sound and ensta-paris all refused in one pass) were reported blocked
**before** any design was started, per CLAUDE.md §9's standing instruction in
the user's words, and the user fetched them plus the complete Fletcher &
Rossing into `technical references/drumsynth/new/`. Everything taken is physics or a documented mechanism; the one table of
numbers copied — the cymbal generator's oscillator frequencies — is attributed
again at the point of use.

| Source | Licence / status | Used for |
|---|---|---|
| Kurt James Werner, Jonathan S. Abel, Julius O. Smith, "A Physically-Informed, Circuit-Bendable, Digital Model of the Roland TR-808 Bass Drum Circuit", **DAFx-14**, Erlangen 2014 | Conference paper — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-02** | The mechanism behind the kick engine's two pitch effects, kept separate as the paper separates them: the **≈6 ms attack frequency jump of more than an octave** (Q43 raising the bridged-T's centre and Q — heard as "punchier and crisper", not as pitch), and the slow **pitch sigh** (leakage through R161 once the output swings below a diode drop). Also the **retrigger pulse** (C39/R161/D52) that stops the amplitude jumping when the centre returns, the output stage's shape (passive low-pass tone → level divider → high-pass), the bridged-T's ≈49.5 Hz centre against the 56 Hz service-chart figure, the absence of a machine-gun effect on fast retriggers, and unit-to-unit variation as part tolerance. Ictus does not model the circuit: its body is a phase-continuous sine under two `TensionDrop`s, and the paper is what says which two. |
| Kurt James Werner, Jonathan S. Abel, Julius O. Smith, "The TR-808 Cymbal: a Physically-Informed, Circuit-Bendable, Digital Model", ICMC/SMC 2014 | **CC BY 3.0** (stated on the paper). **READ first-hand, user-supplied PDF, 2026-09-02** | The metal generator the hat engine's ***Metal*** ratio set is built from (named for what it is, not for the product analysed -- CLAUDE.md §2.1) — the one table taken under §9's copy-what-measurement-cannot-check rule: **six rectangular oscillators at 205.3, 369.6, 304.4, 522.7 Hz and two tunable ones nominally 800 Hz (359.4–1149.9) and 540 Hz (254.3–627.2), duty 47.98 %**, summed into two band-passes centred ≈3440 Hz and ≈7100 Hz, then envelopes, swing-type VCAs and three Sallen-Key high-passes. Three of those numbers are in the code: the six frequencies as `kMetalHz`, the duty as `kDutyCycle`, and 7100/3440 as `kUpperBandRatio`, which is how far the hat's upper band-pass sits above Colour. Attributed again in `plugins/Ictus/Dsp/HatEngine.hpp`. The Nord Modular chapter's overdrive after the band-pass is the hat's *Drive*, and its "many incommensurate oscillators" is what the *Ring* control multiplies into a dense wash. Also the paper's own practice of rendering at 4× oversampling because the rectangular waves alias — which is why Ictus's pulses are polyBLEP inside the oversampled section rather than naive squares. |
| Gordon Reid, *Synth Secrets*: "Synthesizing Drums: The Bass Drum", **Sound On Sound**, Jan 2002 | Magazine article — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-02** | A real kick's partials as a harmonic series shifted by a constant (43 Hz + 7 Hz → 50, 93, 136, 179 Hz — parked as a roadmap item), the pitch dropping "a couple of semitones from start to finish" (the *Sigh* range), one VCA+EG being enough for the decay, and the beater click as a burst of short-lived high-frequency energy — the click resonator plus noise burst. |
| Gordon Reid, *Synth Secrets*: "Synthesizing Drums: The Snare Drum", **Sound On Sound**, Feb 2002 | Magazine article — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-02** | The snare engine's default mode ratios: two quasi-harmonic series (spacings 101–125 Hz, fitted at 111 Hz with offsets 175 and 224 Hz) plus the **0,1 pair at ≈180 and ≈330 Hz that decays far faster** — hence 1 : 1.6 : 2.2 with shorter upper T60s. The wires as noise through VCA+EG, and the two velocity mappings the engine adopts: **velocity → noise filter cutoff** and **velocity → tuned/noise mix** (the article's Figures 13–14). Attributed again at the point of use, `plugins/Ictus/Dsp/SnareEngine.hpp` (I3), which rounds the ratios to 1 : 1.6 : 2.2 and measures them back at 1 : 1.601 : 2.201. |
| Gordon Reid, *Synth Secrets*: "Synthesizing Percussion", **Sound On Sound**, Nov 2001 | Magazine article — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-02** | Membrane mode physics (the 0,1 fundamental, air loading sharpening the modes upward), why cymbals and hi-hats are unpitched, and the initial-noise-then-ring structure. Background for the perc pad and the hats; nothing numeric taken. |
| James Clark, "Percussion", ch. 5 of the *Nord Modular* tutorial book (cim.mcgill.ca/~clark/nordmodularbook) | Web book — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-02** | The resonant-filter kick rung by a narrow pulse, oscillator sync so every hit starts at the same phase (the kick's *Phase* control), metal as many incommensurate oscillators or ring-mod/FM, the 808-style cymbal patch (oscillators → band-pass → overdrive), and **the clap as white noise amplitude-modulated by a fast envelope fired by four pulses ≈11 ms apart** — the clap engine's default spacing. |
| **N. H. Fletcher & T. D. Rossing, *The Physics of Musical Instruments*, 2nd ed. (Springer)** -- §3.6 Circular Plates, ch. 20 Cymbals, Gongs, Plates | Book -- cite, do not paste. **READ first-hand, user-supplied complete PDF, 2026-09-05** (`technical references/drumsynth/new/`) | The hat's **Plate** layer. **Taken** (a published measurement no measurement of ours could check): Table 20.1's fit for the 14-inch thick cymbal, `f = c m^p` with **p = 1.47** -- `HatEngine::kPlateExponent`. Read, not taken: Chladni's law `f = c (m + 2n)^p` with p = 2 for flat plates and p < 2 for cymbals (§3.6); Table 3.2's free-plate ratios; the two-slope fits for larger, thinner cymbals (p 1.65-1.86 then 1.43-1.56); §20.3's transient -- the 2-10 kHz band builds by 10 dB or more in the first 50 ms, sub-700 Hz drains in 200 ms, 3-5 kHz shimmer dominates 1-4 s; §20.6's coupled-mode equations (20.1-20.6), the physics of the parked build-up. Attributed again in `plugins/Ictus/Dsp/HatEngine.hpp`. The rest of the book's bearing on the suite is tabled in `plugins/Ictus/PLAN.md`, "What else the book bears on". |
| R. Perrin, G. M. Swallowe, S. A. Zietlow, T. R. Moore, "The normal modes of cymbals", Proc. Inst. Acoust. 30(2), 2008 (Rollins Scholarship Online, open) | Conference paper -- cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-05** | Measured modes: 18-inch crash (3,0) 64, (4,0) 99, (5,0) 145, (6,0) 188, (8,0) 285, (23,0) 1864 Hz, the {(2,1)+(9,0)} cluster at 339-354 Hz; 12-inch ridged (2,0) 62, (3,0) 135, (5,0) 459, (10,0) 971, (13,0) 1421 Hz, (1,1) mixed with (8,0) at 781 Hz. Q of 10³-10⁴ by the decay method; most modes are split doublets; mixing only between near neighbours and by perturbation, not nonlinearity, which appears above 20 µm. Where the plate's circle-family offset **β = 7.4** (`kPlateCircleOffset`) was read off -- design informed by these two cymbals, not a copied number -- and why the plate carries a deterministic jitter in place of doublets. |
| A. Chaigne, C. Touzé, O. Thomas, "Nonlinear vibrations and chaos in gongs and cymbals", Acoust. Sci. & Tech. 26(5), 403-409, 2005 (HAL hal-01135295, open) | Journal paper -- cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-05** | The cascade's mechanism: geometric quadratic nonlinearity from curvature; combination resonances `pΩ = a_i ω_i + a_j ω_j`, `|a_i| + |a_j| = 2`; 1:1:2 internal resonance; the dynamics governed by 5-10 active modes; periodic → quasiperiodic → chaotic under rising drive, the chaotic regime low-dimensional and deterministic (correlation dimension converges; a positive Lyapunov exponent ≈ 0.02/sample); pitch glide is a thick-gong effect, thin cymbals cascade first. The physics the shared bank's Bloom approximates and the reason it was tried on the plate -- and, measured, removed (`plugins/Ictus/PLAN.md`, I4.3). |
| M. Karjalainen, H. Järveläinen, "Reverberation Modeling Using Velvet Noise", AES 30th Int. Conf., 2007 (and the DAFx velvet-noise papers after it) | Conference papers -- cite, do not paste. **NOT READ: not fetched.** The proxy refuses dafx.de and aes.org; known second-hand from the literature's summaries. Reported to the user per CLAUDE.md section 9 with the Ictus I4.5 push | The shape of `dsp::EarlyReflections` -- sparse random-sign taps at random positions within equal cells -- is the velvet-noise idea. **Nothing numeric is taken**: the tap count (48 a side), the 30 dB fall, the unit-energy normalisation and the tone are this project's, chosen and measured here. If the user fetches the paper, the row and the class get re-read against it. |
| **N. H. Fletcher & T. D. Rossing, *The Physics of Musical Instruments*, 2nd ed.** -- §18.7-18.13, drums with two heads, snare action | Book -- cite, do not paste. **READ first-hand, 2026-09-05** (`technical references/drumsynth/new/`, the complete text) | **Taken** (a measurement no measurement of ours could check): **Table 18.7's tom-tom mode ratios 1 : 2.16 : 3.14** as `SnareEngine::kTomRatios`, the far end of the snare's *Head* control. **Read, not taken**: §18.13's snare action -- the snares leave the head only above a critical amplitude that rises with their tension, and a soft blow at high tension does not rattle (Fig. 18.10) -- which is the mechanism *Tension* implements as a threshold on the head's normalised motion; Table 18.6's side-drum modes; the bass drum's near-harmonic series; the tom's pitch glide of about 8 % over the first 0.2 s. Attributed again in `plugins/Ictus/Dsp/SnareEngine.hpp`. |
| M. Ducceschi, C. Touzé, "Modal approach for nonlinear vibrations of damped impacted plates: Application to sound synthesis of gongs and cymbals", J. Sound Vib. 344, 313-331, 2015 | Journal paper (Elsevier) -- cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-05** | **Taken**: the modal damping law's exponent for the cymbal case, `c_p = 0.007 ω_p^0.7` -- so T60 ∝ ω^-0.7, `HatEngine::kPlateDampExponent` (the gong case is 0.005 ω^0.6). Read: the stick as a raised-cosine force, half-width 1 ms hard / 5-10 ms soft; cymbal a = 0.2 m, h = 0.5 mm, f₁ = 15.5 Hz, f₁₀₀₀ = 11 495 Hz, energy to 20 kHz within milliseconds of a 120 N strike; ~1000 modes and about 3 h of CPU per second of sound for the full nonlinear model -- the figure that makes 64 linear modes plus a noise layer the honest real-time budget rather than a corner cut. |
| J. J. Harrison, A. J. Hill, "A scientific approach to microphone placement for cymbals in live sound", Proc. Inst. Acoust. 35(2), 2013 | Conference paper -- cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-05** | Nothing numeric taken. Polar measurements of five B20 cymbals including **14-inch hi-hats, open and closed**: omnidirectional at 5 ms, the 100-250 Hz of the early attack giving way to rising highs between 25 and 500 ms, more so on the heavier cymbals -- the build-up is a hat property too, which is what put it on the roadmap rather than in the bin. |
| Gordon Reid, *Synth Secrets*: "Analysing Metallic Percussion", **Sound On Sound**, May 2002 | Magazine article -- cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-05** | "Hundreds of energised modes ... at discrete frequencies"; the strike's few ms → strong peaks at a few hundred Hz → energy moving up to a few kHz over hundreds of ms → the highs die first; at high amplitude the spectrum is "in essence noise", so the analogue drum-machine designers "were not completely wrong to use white noise". Background for the plate and the noise layer; nothing numeric taken. |
| Trevor Page's 9090 (TR-909 clone) as described on nuxx.net; the TR-909 bass drum PCB page; the TR-909 bass drum schematic image | Web pages and a schematic scan — cite, do not paste. **READ first-hand, user-supplied PDF/JPG, 2026-09-02** | The 909 kick's block structure, which is what the kick engine's knob set follows: a triangle VCO rounded by a diode clamp (the 9090's *Distortion* overdrives the clamp — the *Harmonics* control), a pitch envelope with *Pitch*, *Tune Decay* (sweep speed) and *Tune Depth* (amount) — *Drop* and *Drop time* — and a separate filtered **noise click whose level is the Attack knob**, plus Decay and Level. No circuit values are used; the body is a sine shaped by ADAA, not a modelled VCO. |
| Image-Line, "Multi Input / Output Plugins" (2010) and the FL Studio manual's *Plugin Wrapper* page (Processing and Connections sections) | Manufacturer documentation — public. **READ first-hand, user-supplied PDF, 2026-09-02** | How FL Studio routes a multi-output instrument: *Auto map outputs* assigns plugin outputs to the mixer tracks **following the plugin's own track**; manual assignment is in **mixer-track offsets** relative to that track; FL **does not process outputs a plugin marks inactive** unless "Process inactive inputs and outputs" is on — the reason every Ictus bus is declared enabled from the constructor. |
| JUCE forum threads: "Multi-out Synth" (2019), "FL Studio Bus Configuration issue" (2024), "Creating multiple bus outputs for an audio plugin — how it works in DAW?" (2026); JUCE `examples/Plugins/MultiOutSynthPluginDemo.h` (local JUCE 9.0.1 tree) | Forum posts and JUCE's own example (JUCE's licence covers the example). **READ first-hand, 2026-09-02** (the threads as user-supplied PDFs, the example from the fetched JUCE tree) | Aux buses declared *disabled* left only the first bus playing in some hosts until the user enabled pins by hand; working constructors declare every bus enabled. The `isBusesLayoutSupported` shape (no inputs; each output bus disabled or stereo). |
| Stefan Bilbao, "Time domain simulation and sound synthesis for the snare drum", **JASA 131(1):914–925**, 2012 | Journal paper — **abstract only** (paywalled; not fetched) | Understanding only: a full three-dimensional finite-difference snare with membrane–wire collisions is far beyond a plugin voice, and the abstract's point that the wire interaction is what modal methods struggle with is why the snare engine keeps exactly one nonlinearity — the *Rattle* follower — and nothing else from that model. |
| Geonkick (quamplex/geonkick), README only | **GPLv3** (not "or later"; compatible with AGPLv3). Read first-hand; **no code read or copied** | Architecture reference only: confirms a per-kit output map is a normal thing for a drum plugin to offer. |
| James A. Moorer's own hand-drawn sketch of the structure of a 1982-83 cinema logo theme, published by the rights holder for its 35th anniversary | Published promotional material — **a specification read, never copied**. **READ first-hand, user-supplied image, 2026-09-03**. §2.1: reading published documentation to understand what something *does* is expressly allowed; the artwork, the branding and the name are not used anywhere and the preset carries a house name. | The structure of Sonitus's **Conflux** preset, and the correction of a design that had it backwards. The sketch states: thirty voices at random pitches between **200 and 400 Hz**, each moving "slowly and randomly", then "all voices proceed direct to target note"; **three voices per note, slightly detuned**, and **two per note in the bass**; quarter note = 60; dynamics mf rising to fff. So the gesture is **narrow-to-wide**, and the perceived convergence is that every voice stops moving at the same instant. The first version of the preset started wide and converged inward, which is the opposite. Also the source of the honest limitation recorded in `docs/WHATS-NEW.md`: a voice's starting pitch is independent of its target, which needs key tracking *multiplied* by an envelope and is therefore not reachable from one instance of this matrix. |

## FM synthesis — Stryda

Stryda builds its operators from the published mathematics of frequency and
phase modulation, and never from any product's binary, presets or panel. Every
source below was **blocked at the network layer** on the first attempt — ten
domains refused (`ccrma.stanford.edu`, `web.eecs.umich.edu`, `web.uvic.ca`,
`www.dafx.de`, `arxiv.org`, `en.wikipedia.org`, `electricdruid.net`,
`yamahasynth.com`, `tildearrow.org`, `mural.maynoothuniversity.ie`) — and so did
**Google Drive on all four of its hosts** (`drive.google.com`,
`drive.usercontent.google.com`, `docs.google.com`, `googleusercontent.com`) when
the user tried that route after GitHub refused the upload for size. `curl` was no
help; the block is the proxy, not the tool. Per CLAUDE.md §9 the URLs were listed
for the user, who uploaded the papers to `technical references/stryda/` in two
sessions; they were **read first-hand from there on 2026-09-04**. That access
route is part of the record.

Two things are taken under §9's copy-what-measurement-cannot-check rule — the
ModFM extension's closed form and the three aliasing criteria — and both are
attributed again at the point of use.

| Source | Licence / status | Used for |
|---|---|---|
| John M. Chowning, "The Synthesis of Complex Audio Spectra by Means of Frequency Modulation", **JAES 21(7)**, 1973 | Journal paper — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-04** | The foundation: side frequencies at `f_c ± n·f_m` with amplitudes `J_n(I)`, "the higher the order of the side frequency the larger the index must be for that side frequency to have significant amplitude", and `BW ≈ 2(d + m)`. The point that a magnitude-only reading gets wrong: **negative-frequency components reflect around 0 Hz with an inversion of phase and add algebraically** to what is already there, which is why low `c:m` ratios sound as they do. Harmonicity from `c/m = N₁/N₂` rational. This is the sharpened `k_max ≈ I + 2·I^⅓ + 1` bound's parent; the bound itself is standard Bessel asymptotics, derived here and measured in F1, not taken from the paper. |
| Victor Lazzarini and Joseph Timoney, "Theory and Practice of Modified Frequency Modulation Synthesis", **JAES 58(6)**, 2010, pp. 459–471 | Journal paper — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-04** | Three things, all taken as closed forms. **(a)** The Extensions section's `x(t) = e^(r·k·cos ω_m t − r·k)·cos(ω_c t + s·k·sin ω_m t)`, `0 ≤ r ≤ 1`, `−1 ≤ s ≤ 1`, with spectrum `½ ΣΣ I_a(rk)·J_b(sk)·[…]` — which is Stryda's **Character** control: `r=0, s=1` is exactly classic PM, `r=1, s=0` is ModFM, and between them the spectrum goes asymmetric. **(b)** Eq (12), the **maximum index that avoids aliasing**: the largest `k` with `20·log₁₀(Iₙ(k)/I₀(k)) ≤ −60 dB`, `n = (f_sr/2 − f_c)/f_m`. **(c)** Eq (13)/(14), the **phase-synchronous formant operator**: `f_c = n·f_m` with `n = int(f_f/f_m)`, two carriers at `n` and `n+1` crossfaded by `a = f_f/f_m − n`, plus a frequency shift `ω_s` for inharmonicity with the ratio still locked. Also its harmonicity rules (`N₂ = 1` → all harmonics, integral `N₂ > 1` → every `N₂`-th missing) and the observation that ModFM needs an index ~50 % higher than classic FM for the same steady-state brightness — measured in F1 rather than assumed. |
| Joseph Timoney, Victor Lazzarini and Thomas Lysaght, "A Modified FM Synthesis Approach to Bandlimited Signal Generation", **DAFx-08**, Espoo | Conference paper — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-04** | The bandlimited-pulse form `y(t) = e^(k·cos ωt − k)·cos(ωt)`, and the reason it earns a place on a bass synthesiser: modified-Bessel partials "change smoothly from one to the next without the unexpected appearance of insignificant partials that is characteristic of the original FM synthesis", so moving the index gives "a perceptual effect more reminiscent of an opening/closing lowpass filter". That sentence is why Character exists as a knob rather than a mode. |
| Joseph Timoney and Victor Lazzarini, "Exponential Frequency Modulation Bandwidth Criterion for Virtual Analog Applications", **DAFx-11**, Paris | Conference paper — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-04** | The −80 dB bandwidth criterion for **exponential** FM (analogue cross-modulation), which is a *different* technique from linear/phase FM and therefore de-risks an extra mode rather than correcting the main one: `BW₋₈₀dB = f_c·e^(V₀ln2)·2^(V_m−1)·p₀₁ + (p₁₁ + V_m)·f_m` with `p₀₁ = 2.771`, `p₁₁ = 4.3030`, fitted from `p₀ = 2.771 / 5.168 / 9.384` and `p₁ = 4.303 / 6.461 / 9.249` at `V_m = 1 / 2 / 3`. The paper notes the fit deliberately over-estimates rather than under-estimates, which is the right direction for a safety bound. Also the useful confirmation that a −80 dB threshold is "much stronger than Carson's rule" — the threshold Stryda's own measurements use. |
| Victor Lazzarini et al., "Theory and practice of higher-order frequency modulation synthesis", **Journal of New Music Research**, 2024 (arXiv 2305.07909) | Journal paper — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-04** | True (frequency, not phase) modulation at second order and above, with an operator formulation that composes into arbitrary topologies including feedback. Read and understood; **parked** as a candidate seventh character rather than built, because Character already spans the two flavours the brief asked for. `docs/ROADMAP.md`. |
| Tom Wiltshire (Electric Druid), "Phase Distortion Synthesis", 2008 | Web article — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-04** | The identity that saved an operator mode: **phase distortion is phase modulation with a piecewise (knee/triangle) modulator at the carrier's own frequency, or a simple multiple of it.** So `dsp::PhaseShaper` is a phase-transfer function applied at the operator's read rather than a separate synthesis engine, and its header says so. Also the point that PD's extra richness comes from the modulator's shape rather than its frequency. |
| Yasuji Tomisawa, "Tone production method for an electronic musical instrument", **US Patent 4,249,447**, 1981 | Patent — public document; the technique is long expired. **READ first-hand, user-supplied PDF, 2026-09-04** | First-hand confirmation of the feedback-operator rule `shared/tezla-dsp/include/tezla/dsp/Oscillator.hpp` already implements and previously cited second-hand: averaging the operator's last two outputs before feeding them back "eliminates the hunting" — the alternating-sample oscillation at Nyquist that an unaveraged single-sample loop produces. The citation in that header moves from *documented behaviour of a published instrument* to *read*. |
| Jon Dent, "DX 7 — Keyboard Level Scaling", *Exploring Electronic Music*, 2019 | Web article — cite, do not paste. **READ first-hand, user-supplied PDF, 2026-09-04** | The key-scaling structure Stryda's operators adopt: a **break point** anywhere from A1 to C8; **four curve types** (linear and exponential, positive and negative) chosen **independently left and right** of it; **depth 0–99** with 0 flat and 99 maximum. And the distinction that makes it musical rather than decorative — scaling a **modulator** changes timbre up the keyboard, scaling a **carrier** changes level. |
| **Miller's downward recurrence** for `J_n(x)` — classical numerical analysis, described in Abramowitz & Stegun §9.12 and in *Numerical Recipes* (`bessj`) | Method, not code. **NOT read — derived here from the recurrence and the normalisation identity, both standard, then verified against this repository's own integral evaluator.** The two books were not consulted; neither is fetchable from this container and neither was needed, because the method is two published identities and a scaling choice. | `dsp::besselJLadder`. Bessel's recurrence `J_(n-1)(x) = (2n/x)·J_n(x) − J_(n+1)(x)` is unstable upwards for `n > x` and stable downwards, so the ladder seeds an arbitrary value above the turning point, walks down, and fixes the scale with `J_0(x) + 2·(J_2(x) + J_4(x) + …) = 1`. Written because `fm::significantOrderExact` needs *every* order up to the aliasing edge and evaluating them one at a time is quadratic — 5.7 ms at an index of 64 radians, which froze FL Studio on the rig (2026-09-04). Agrees with `besselJn`'s integral to **2.232e-15** at worst over `x` up to 200 and orders up to 236, and is ~6600× faster; the disagreement figure and the timing are both pinned in `tests/test_FmBandwidth.cpp`. Since a measurement *can* tell us this is wrong, §9 says derive it — which is what happened. |
| Computer Music / MusicRadar, "How to make a growling bass sound using FM synthesis" and "How to create a tearing neurofunk DnB Reese sound"; BassGorilla, "What is Neurofunk" | Magazine and web articles — cite, do not paste. **READ first-hand, user-supplied PDFs, 2026-09-04** | Production practice only, for preset voicing and the ratio sequencer's defaults: rhythmic modulation of the index or wavetable position at 1/8 to 1/16 locked to the drums, short looping envelopes used as modulators, serial distortion with dynamic band-pass/notch/comb filtering, and the consistent observation that the genre's basses are made by *resampling and reprocessing* rather than in one synth pass — which is why the mangle chain and the protected sub lane are part of the instrument rather than an afterthought. No sound, preset or product setting is taken from any of them. |
