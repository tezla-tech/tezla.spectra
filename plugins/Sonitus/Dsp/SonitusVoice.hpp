...
public:
                        ...
                        double pitchModulationCents, double levelModulation)
    {
        ...
        const double pitchRatio = std::pow (2.0, pitchModulationCents / 1200.0);

        const double nominalA = frequency_ * pitchRatio
                                  * ratioFor (parameters.octaveA, parameters.semitonesA,
                                              parameters.centsA);

        configureBank (bankA_, parameters.shapeA, parameters.widthA, parameters.unisonA,
                       parameters.detuneA, parameters.spreadA, parameters.driftA, nominalA);

        // A's nominal pitch, not any one of its detuned oscillators -- see
        // propagateSync().
        syncIncrement_ = nominalA > 0.0 ? nominalA / sampleRate_ : 0.0;

        configureBank (bankB_, parameters.shapeB, parameters.widthB, parameters.unisonB,
                       parameters.detuneB, parameters.spreadB, parameters.driftB,
                       frequency_ * pitchRatio
                         * ratioFor (parameters.octaveB, parameters.semitonesB, parameters.centsB));
        ...
        for (auto& filter : filters_)
        {
            ...
            filter.setDrive (parameters.filterDrive);
        }
        ...
        folder_.setGain (dsp::isExactlyZero (parameters.foldAmount)
                           ? 0.0
                           : std::pow (kMaximumFold, parameters.foldAmount));

        cutoff_.setTarget (targetCutoff (cutoffModulation));
        gain_.setTarget (targetGain (levelModulation));
    }
    ...
    void process (double& left, double& right) noexcept
    {
        ...
        const double phaseMod = parameters_.pmIndex * modulator;

        bankB_.process (phaseMod, leftB, rightB);

        const double subValue = sub_.advance() * parameters_.subLevel;

        double mixLeft = leftA * parameters_.levelA + leftB * parameters_.levelB;
        double mixRight = rightA * parameters_.levelA + rightB * parameters_.levelB;

        if (! dsp::isExactlyZero (parameters_.ringAmount))
        {
            ...
            mixLeft += parameters_.ringAmount * (ringLeft - mixLeft);
            mixRight += parameters_.ringAmount * (ringRight - mixRight);
        }

        if (! dsp::isExactlyZero (parameters_.foldAmount))
        {
