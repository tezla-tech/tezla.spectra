...
| `Tzic` | [`Ictus`](Ictus/) | Tezla Ictus | **Instrument** — the drum synthesiser for drum and bass (Latin *ictus*: the stroke, and in music the beat itself). Purpose-built kick, snare, hat and clap engines derived from the published circuit analyses and membrane physics (never from a product), a per-pad punch chain — transient, drive, weight and tilt, one-knob smash, clip — that builds in the layering the user did by hand, per-hit humanise and velocity, a sample layer with onset alignment and Render-to-WAV, and per-pad output buses for FL Studio's Auto map outputs. Plan: [`Ictus/PLAN.md`](Ictus/PLAN.md) | **in progress** — I3: the kick and the snare (v0.1.0; Perc and Snare 2 on the snare engine's defaults), builds and passes the validator 47/47; the I2 kick **has been played on the rig**, whose first ear round added Bass mode, Gate + Release and the tuning page; the snare has not been played there yet |
