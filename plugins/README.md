...
| `Tzic` | [`Ictus`](Ictus/) | Tezla Ictus | **Instrument** — the drum synthesiser for drum and bass (Latin *ictus*: the stroke, and in music the beat itself). Purpose-built kick, snare, hat and clap engines derived from the published circuit analyses and membrane physics (never from a product), a per-pad punch chain — transient, drive, weight and tilt, one-knob smash, clip — that builds in the layering the user did by hand, per-hit humanise and velocity, a sample layer with onset alignment and Render-to-WAV, and per-pad output buses for FL Studio's Auto map outputs. Plan: [`Ictus/PLAN.md`](Ictus/PLAN.md) | **in progress** — I4.1: the kick, the snare, the ghost snare, both hats (dense, damped, gated, with the hiss rung through the metal) and the clap (a burst pattern over a cavity that rings) (v0.1.0; Perc on the snare engine's defaults), Note snap, the plated panel with its pictures, builds and passes the validator 47/47; the I2 kick **has been played on the rig**, whose first ear round added Bass mode, Gate + Release and the tuning page; the hats were heard at I4.1 ("thin and tinny") and gained **Plate** -- a 64-mode cymbal from the published mode law -- and **Grit** at I4.3; the fourth round (I4.4) added **Under** and **Knock** on the kick, **Ring** and **Thump** on the snares, **Air tilt / Air attack / Grain / Vel > Air** and an **Open hold** on the hats, and a **Pan** per pad on a MIX page; nothing since I4.1 has been played there yet |
| `Tzst` | [`Stryda`](Stryda/) | Tezla Stryda | **Instrument** — the FM synthesiser for neurofunk drum and bass: six operators in a 6x6 index matrix with feedback on the diagonal and a noise row (which contains every fixed six-operator algorithm of the era as a special case), each operator carrying one **Character** control that runs continuously from classic phase modulation to ModFM — the published `exp(r k cos)` extension, whose partials fall away smoothly so the index behaves like a filter opening — plus a phase-distortion bend, an exponential-FM cell switch and a phase-synchronous formant mode. A **protected sub lane** that never enters the braid or the distortion, a **ratio step sequencer** that jumps harmonic identity in time with the drums, a **vowel lane**, the sixteen-point ADV envelopes, unison **index** spread, and a **bandwidth predictor** built from three published aliasing criteria that says live where the top of the spectrum is and can cap the index so it never crosses Nyquist. Microtuning reaches **the ratio**, not only the keyboard: an operator's ratio can snap to the loaded scale's degrees, so the whole sideband ladder lands in the session's tuning. Plan: [`Stryda/PLAN.md`](Stryda/PLAN.md) | **feature-complete, validating 47/47** — F0 to F9b: the six operators with a waveform choice each and a live wave strip that runs the real operator, the matrix, the index cap, phase distortion, the formant mode, key scaling and velocity, the per-voice filter, the protected sub lane, unison with index spread, microtuning at the ratio with three ratio modes, the ratio sequencer and six braids, Split with the vowel lane and mangle chain, the modulation layer (two ADV envelopes, two LFOs, four macros, eight slots), twelve presets with notes, a NOTES page and a spectrum with the predicted edge drawn on it. 397 parameters, dice gate at 0 unclassified, presets gate at 0 clipping. **Nothing past F3 has been played on the rig.** |
