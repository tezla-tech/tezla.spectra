...
| `Tzic` | [`Ictus`](Ictus/) | Tezla Ictus | **Instrument** — the drum synthesiser for drum and bass (Latin *ictus*: the stroke, and in music the beat itself). Purpose-built kick, snare, hat and clap engines derived from the published circuit analyses and membrane physics (never from a product), a per-pad punch chain — transient, drive, weight and tilt, one-knob smash, clip — that builds in the layering the user did by hand, per-hit humanise and velocity, a sample layer with onset alignment and Render-to-WAV, and per-pad output buses for FL Studio's Auto map outputs. Plan: [`Ictus/PLAN.md`](Ictus/PLAN.md) | **in progress** — I2: the kick pad ships (v0.1.0, kick only), builds and passes the validator 47/47, and **has been played on the rig**; the first ear round added Bass mode, Gate + Release and the tuning page (not yet played there) |
