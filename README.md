...
| **[Ictus](plugins/Ictus/)** | **Instrument** — the drum synthesiser for drum and bass, in progress: the kick (a sine body under the two pitch drops the TR-808 analysis separates, parallel odd/even harmonics through ADAA, a pitch-tracking tone, click and noise, a tail) and the snare (a three-mode shell at the published ratios under a tension drop, wires that rattle with the shell, the stick's crack), with the hats, clap, punch chain, humanise, sample layer and per-pad outputs to follow | v0.1.0 (kick and snare) — 47/47 on Steinberg's validator; the kick played on the rig, whose first ear round added Bass mode, Gate + Release and the tuning page; the snare not yet |

1034 framework-free DSP tests pass on Linux x86-64, which is where development
